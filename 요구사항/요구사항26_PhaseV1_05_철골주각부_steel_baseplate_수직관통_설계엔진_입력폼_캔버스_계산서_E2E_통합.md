...
3. **[Pane 2 모듈 주도형 서브탭 & 원본 1:1 서브 모달]**:
   - 원본 `IDD_STL_USBP_PMODE_DLG` 기반 모듈 주도형 서브탭 정의 (단면/플레이트, 기초/페데스탈, 앵커/보강리브, 설계하중). 통일된 공학 테마와 UX 공유.
...
4. **[Pane 3 동적 가변 1~3단 세로 적층형 그래픽 뷰포트]**:
   ...
   - **[1단 뷰포트: 원본앱 참조 타이틀 `베이스플레이트 상세 평면도`]**: 베이스플레이트 외곽선($B \times N$), H/Box 기둥 단면 배치선, 앵커볼트 위치 홀(원형 심볼), 연단거리/볼트간격 치수선, 리브 스티프너.
   - **[2단 뷰포트: 원본앱 참조 타이틀 `주각부 입면 및 지압응력도`]**: 주각부 정면 입면도, 기둥-플레이트-그라우트-페데스탈 접합단면, 앵커 매립 형상, 콘크리트 지압응력 분포 다이어그램 오버레이.
...
* **원본앱 3대 액션 버튼 툴바**:
  ...
  - `[✨ 자동설계]`: 현재 하중 조건에 적합한 최적 베이스플레이트 두께($t_p$) 및 앵커볼트 규격 자동 최적화 추천.
* **상세 대화창(`...`) 서브 모달 3종**:
...
* **4-Pane 실시간 연동**: 폼 입력 시 입면도/평면도 그래픽 즉시 동기화, `[⚡ 검토]` 클릭 시 100ms 이내 계산서 및 지압/두께/앵커 DCR 안전율 갱신.
...
- [ ] **1:1 입력폼**: 원본앱 `IDD_STL_USBP_PMODE_DLG` 4대 서브탭, 3대 액션 버튼(`[💾적용] [⚡검토] [✨자동설계]`), 서브탭 래핑 및 테마 무결성 브라우저 렌더링 확인.
