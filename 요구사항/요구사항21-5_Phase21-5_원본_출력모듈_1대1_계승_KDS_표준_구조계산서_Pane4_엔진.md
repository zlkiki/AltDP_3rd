...
- [ ] 다크 테마 상태에서도 계산서 용지 내부가 순백색(`#ffffff`)과 고대비 글자로 유지되는가?
- [ ] `상세 보고서` 선택 시 KaTeX 수식 전개 과정(기호 → 수치대입 → 결과)이 완벽히 렌더링되는가?
- [ ] `사용자 입력 데이터 상세 포함` 체크 해제 시 제2장 입력 데이터 표가 계산서에서 즉시 숨겨지는가?
- [ ] 머릿말 설정 모달에서 회사명/프로젝트명을 입력하면 계산서 최상단 헤더에 즉각 반영되는가?
- [ ] 종합 판정란에 원본앱 스타일의 `  →  O.K` (녹색) 또는 `  →  N.G` (적색)가 정확히 표기되는가?
