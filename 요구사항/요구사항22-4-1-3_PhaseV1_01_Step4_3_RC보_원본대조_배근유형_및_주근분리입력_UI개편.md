...
- [x] Tab 2 배근 탭 상단에 배근 유형 3종 라디오가 정상 배치되고 초기값이 `SYMMETRIC_ENDS`로 설정될 것.
- [x] 배근 테이블 12개 주근 셀이 [개수 텍스트박스 + '-' 라벨 + 호칭경 콤보] 복합 컨트롤로 정상 렌더링될 것.
- [x] 1단 개수 최소 2개(`min="2"`), 2단 개수 0개(`min="0"`) 제약이 정상 작동할 것.
- [x] 2단 개수 `0` 입력 시 호칭경 콤보 디밍 및 데이터 모델 `"0"` 정규화 처리가 정상 수행될 것.
- [x] 개수 또는 콤보 변경 시 `{count}-{dia}` 결합 문자열이 생성되어 캔버스 및 계산서와 실시간 동기화될 것.
- [x] 브라우저 창/패널 리사이즈 시 배근 테이블 열이 겹치거나 깨지지 않고 가로 스크롤로 안전하게 방어될 것.
- [x] 브라우저 콘솔 에러가 0건일 것.
