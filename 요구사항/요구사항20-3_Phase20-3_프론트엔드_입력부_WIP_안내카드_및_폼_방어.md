...
* **상위 기술 문서(SSOT)**:
  ...
  - [`docs/07_web_application_ui_ux_specification.md`](file:///f:/PyProject/AltDP_3rd/docs/07_web_application_ui_ux_specification.md) (Left-Sub 워크스페이스 & 3버튼 액션 파이프라인)
  - [`docs/13_midas_design_plus_original_ui_specification.md`](file:///f:/PyProject/AltDP_3rd/docs/13_midas_design_plus_original_ui_specification.md) (Midas 원본 다이얼로그 탭 구조)
* **목적**: 1열 탐색기에서 Midas 1:1 서브탭 전용 폼이 아직 연동 준비 중(WIP)인 부재를 선택했을 때, **`docs/07`의 Left-Sub 영역(`pane-input-form`)에 이전 부재의 폼이 잔존하거나 엉뚱한 필드가 노출되는 결함을 완벽히 차단**하고, 모던 글래스모피즘 기반의 **"Midas 전용 WIP 안내 카드"**를 렌더링하며, 상단 툴바의 **3버튼 액션 파이프라인(`적용/검토/설계`)**과 안전하게 연동합니다.
...
### 2.2. Midas 전용 WIP 안내 카드 렌더러 (`WIPCardRenderer`)
...
* **UI 디자인 및 구성 요소 (`docs/07`, `docs/13`)**:
  ...
  4. **지원 예정 서브탭 안내**: Midas 원본 1:1 서브탭 (`[재질 및 단면]`, `[배근 상세]`, `[설계 하중]`, `[토압/수압 조건]`).
  5. **로드맵 알림문**: "본 부재는 KDS 수식 및 Midas 1:1 서브탭 전용 폼 개발 준비 중입니다."

### 2.3. 3버튼 액션 파이프라인 안전 방어 (`docs/07` 제1절)
* 상단 마스터 툴바의 **`[💾 적용 (Apply)]`**, **`[⚡ 검토 (Check)]`**, **`[✨ 설계 (Design)]`** 버튼 클릭 시:
  - WIP 상태인 부재일 경우 가짜 계산 요청을 전송하지 않고:
  - 브라우저 우하단에 **"⚠️ 해당 부재는 Midas 1:1 전용 서브탭 폼 및 KDS 연산 탑재 준비 중입니다."** 토스트 알림 연동.
...
3. **Step 3 [Medium]**: 티어별 뱃지 및 Midas Design+ DLG 코드/KDS 기준 자동 바인딩 로직 구현.
...
* Left-Sub의 사용자 입력부에 부재명, 티어 뱃지, Midas DLG 코드가 기재된 WIP 카드가 렌더링될 것.
