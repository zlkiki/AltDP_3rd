...
- [x] Pane 1에서 부재 추가/복제/삭제 및 인라인 이름 편집이 오류 없이 동작하는가?
- [x] 부재 요약 테이블에서 단면, 배근, 소요력, DCR 글자가 실시간으로 갱신되는가?
- [x] Pane 2에 4대 고유 서브탭이 제공되며, 탭 전환 시 입력 데이터가 손실 없이 유지되는가?
- [x] 서브 대화창 버튼(`...`) 클릭 시 해당 모달(유효폭, 단면DB 등)이 정확히 팝업되고 계산값이 폼에 역주입되는가?
- [x] `[💾 적용]`, `[⚡ 검토]`, `[✨ 설계]` 3버튼 파이프라인이 누락 없이 작동하는가?
