...
* **4대 포팅 참조 우선순위 (SSOT Hierarchy)**:
  ...
  3. `[3순위 학회 예제집]`: 한국콘크리트학회(2020) 콘크리트구조 학회기준 예제집 (`콘크리트구조 학회기준 예제집(2020)_OCR.pdf`)
     - `예제 4.1 (구 3.1)`: 단철근 직사각형 보의 휨모멘트 강도 ($\phi M_n$) 산정 (PDF p.62-63, $\phi M_n = 159.3\text{ kN}\cdot\text{m}$)
     - `예제 4.3 (구 3.2)`: 복철근 직사각형 보의 압축철근 항복 여부 및 휨강도 (PDF p.68-70, $\phi M_n = 617.2\text{ kN}\cdot\text{m}$)
     - `예제 4.5 (구 3.5)`: T형 보의 플랜지 유효폭 및 휨모멘트 강도 (PDF p.78-80, $\phi M_n = 330.0\text{ kN}\cdot\text{m}$)
     - `예제 6.1 (구 4.1)`: 수직 스터럽을 갖는 보의 전단강도 ($\phi V_n$) 및 간격 (PDF p.148-150, $\phi V_n = 288.77\text{ kN}$)
     - `예제 7.1 (구 4.3)`: 비틀림 모멘트 작용 시 전단-비틀림 상호작용 및 종방향 철근($A_l$) 산정 (PDF p.184-186, $A_{l,req} = 644.3\text{ mm}^2$)
     - `예제 3.1 (구 6.1)`: Branson 유효단면2차모멘트($I_e$) 및 단기/장기 복합 처짐 계산 (PDF p.40-42, $\Delta_{i} = 8.08\text{ mm}$)
  4. `[4순위 국가건설기준]`: KDS 14 20 10 (일반/재료), KDS 14 20 20 (휨/압축 표 4.1-2), KDS 14 20 22 (전단/비틀림), KDS 14 20 30 (사용성)
...
1. **등가직사각형 압축응력블록 파라미터 ($\alpha_1, \beta_1$) (KDS 14 20 20:2022 표 4.1-2)**:
   $$\alpha_1 = \eta \times 0.85 = 0.85 \quad (f_{ck} \le 40\text{ MPa})$$
   $$\alpha_1 = \eta \times 0.85 \quad (40 < f_{ck} \le 90\text{ MPa}, \eta = 1.00 \to 0.84)$$
   $$\beta_1 = 0.80 \quad (f_{ck} \le 50\text{ MPa})$$
   $$\beta_1 = 0.80 - 0.004 (f_{ck} - 50) \quad (50 < f_{ck} \le 60\text{ MPa})$$
   $$\beta_1 \ge 0.70 \quad (f_{ck} \ge 90\text{ MPa})$$
