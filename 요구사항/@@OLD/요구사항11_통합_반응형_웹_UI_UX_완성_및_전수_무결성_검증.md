...
### 1.1. 배경
AltDP_3rd의 최종 목표는 원본앱의 모든 설계 엔진을 순수 Python/Web 기반으로 완벽히 마이그레이션하여, 현대적인 브라우저에서 0.05초 이내에 반응하는 인터랙티브 구조설계 환경을 제공하는 것입니다.
...
|:---:|---|---|---|
| **Phase 11-1** | [`요구사항11-1_통합_반응형_웹_대시보드_및_디자인시스템.md`](file:///f:/PyProject/AltDP_3rd/요구사항/요구사항11-1_통합_반응형_웹_대시보드_및_디자인시스템.md) | `src/web/templates/`, `design_tokens.css`, `style.css`, `special.py` | 모던 UI 컴포넌트, 다크모드, 반응형 레이아웃, 특수부재 API |
| **Phase 11-2** | [`요구사항11-2_전_부재_설계_캔버스_통합_인터랙션.md`](file:///f:/PyProject/AltDP_3rd/요구사항/요구사항11-2_전_부재_설계_캔버스_통합_인터랙션.md) | `renderer2d.js`, `pm_chart.js`, `app.js`, 부재별 실시간 캔버스 바인딩 | 파라미터 변경 즉시 캔버스/차트 실시간 동기화 |
| **Phase 11-3** | [`요구사항11-3_전_도메인_회귀테스트_및_0_1퍼센트_무결성검증.md`](file:///f:/PyProject/AltDP_3rd/요구사항/요구사항11-3_전_도메인_회귀테스트_및_0_1퍼센트_무결성검증.md) | `tests/e2e/`, 3대 Pytest 스위트 전체 실행 스크립트 | 전체 pytest 100% 통과 (1.5초 이내) & 무결성 확정 |

---

## 5. 검증 및 수용 기준 (Acceptance Criteria)

- [x] **웹 UI 반응성 및 심미성**: 입력값 변경 시 50ms 이내 재계산 및 캔버스 갱신 완료, 모던 프리미엄 다크/라이트 디자인 구현.
- [x] **3대 Pytest 체계 100% 통과**:
...
- [x] **E2E 전체 파이프라인 무결성**: 단면 선택 $\rightarrow$ 하중 입력 $\rightarrow$ 2D/3D 시각화 $\rightarrow$ A4 구조계산서 출력 전 과정 단절 없는 연속 동작 검증 완료 (150 tests passed in 1.37s).
