...
3. **[Pane 2 4대 서브탭 & 원본 1:1 서브 모달]**:
   - `IDD_RCS_WALL_PMODE_DLG` 1:1 계승 4대 서브탭 (`단면/재료`, `철근배근`, `부재력/전단`, `내진/경계요소`).
   - 서브 모달 2종: `[특수경계요소 상세 모달]`, `[하중조합 모달]`.
   - 3버튼 통합 액션 파이프라인 (`[💾 적용] [⚡ 검토] [✨ 설계]`).
4. **[Pane 3 세로 적층형 2단 그래픽 뷰포트]**:
   - **[상단 뷰포트]**: 벽체 전고($h_w$) 입면도, 층별 분할선, 전단력/모멘트 포락선 다이어그램 오버레이.
   - **[하단 뷰포트]**: 벽체 평단면도($t_w \times l_w$), 복배근 수직/수평근 그리드, 양단부 특수경계요소(SBE) 구속 영역 음영 해치, 단부 집중 주근 및 띠철근/크로스타이, 치수선, 철근 태그, 줌/팬/호버 툴팁.
...
6. **[실시간 반응성 및 성능]**:
   - 파라미터 변경 시 `ProjectStore` 중심 100ms 이내 3-View 동시 동기화 보장.
