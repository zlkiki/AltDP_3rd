...
| **Step 2** | ⚙️ **Medium** | [**요구사항 23-2**](요구사항23-2_PhaseV1_02_Step2_RC기둥_원본앱_1대1_서브탭_입력폼_및_모달.md) | • 원본앱 `IDD_RCS_COLUMN_PMODE_DLG` 1:1 서브탭 폼 (`form_rc_column.js`)<br>• 모듈 주도형 서브탭 정의(개수/명칭 직접 정의, 테마/UX 통일)<br>• 패널 최소폭 320px 및 컨트롤 110px 반응형 래핑 방어<br>• 3대 액션 버튼(`[💾 적용] [⚡ 검토] [✨ 자동설계]`) 파이프라인 (검토 시에만 계산서 갱신)<br>• 배근 상세 모달 (`IDD_RCS_COLM_REBAR_DLG`) 및 P-M 확장 모달 | 브라우저 DOM 정상<br>콘솔 에러 0건 |
| **Step 3** | ⚙️ **Medium** | [**요구사항 23-3**](요구사항23-3_PhaseV1_02_Step3_RC기둥_2D_VDraw_캔버스_배근도_및_PM곡선_인터랙션.md) | • 동적 가변 1~3단 세로 적층형 뷰포트 (`GraphicViewport.setStationCount(2)`)<br>• 1단: 원본앱 참조 타이틀 `단면 배근도` (사각/원형, 135° 절곡 띠대근, 솔리드 주철근, 치수선)<br>• 2단: 원본앱 참조 타이틀 `P-M 상관곡선` (KDS 200 파이버 $\phi P_n-\phi M_n$ 상관곡선 및 하중점 플롯)<br>• 잔존 임시 모듈 완전 배제, 마우스 휠 줌/팬/Fit 및 하중점 호버 툴팁 | Canvas 그래픽스 렌더링<br>인터랙션 무결성 |
| **Step 4** | 🧠 **High** | [**요구사항 23-4**](요구사항23-4_PhaseV1_02_Step4_RC기둥_A4_5대장구분_8단계_KaTeX_구조계산서.md) | • `report_engine.js` 단일화 및 구형 레거시 렌더러 완전 제거<br>• [⚡ 검토], [✨ 자동설계] 클릭 시 정식 KaTeX 계산서 즉시 출력 파이프라인<br>• 원본 5대 장구분 완벽 계승 (개요-부재력-PM-이축휨-전단)<br>• 8단계 Step-by-Step KaTeX 수식 전개식 (`redcr_rc_column.js`)<br>• 순백색(`#ffffff`) A4 용지 인쇄 프리뷰 및 [🖨️ 인쇄] / [📊 Excel 내보내기] 연동 (PDF 제거) | A4 인쇄 레이아웃<br>KaTeX 수식 무결성<br>Excel 내보내기 정상 |
...
- [ ] **[Step 2 입력폼 1:1]**: 원본 `IDD_RCS_COLUMN_PMODE_DLG` 모듈 주도형 서브탭, 320px 최소폭/110px 컨트롤 반응형 방어, 3대 액션 버튼(`[💾적용] [⚡검토] [✨자동설계]`), 배근/P-M 모달 정상 렌더링.
- [ ] **[Step 3 2D 캔버스]**: 동적 가변 세로 뷰포트(1단: 단면 배근도 + 2단: 200 파이버 KDS P-M 상관곡선/설계하중점), 원본 타이틀 주입, 잔존 모듈 배제, 줌/팬 인터랙션 정상 작동.
- [ ] **[Step 4 A4 계산서]**: `report_engine.js` 단일화, 순백색(`#ffffff`) A4 용지 고정, 5대 장구분 8단계 KaTeX 수식 전개식, [검토/자동설계] 즉시 출력 연동, `  →  O.K / N.G` 판정, [🖨️ 인쇄] 및 [📊 Excel 내보내기] 정상 작동 (PDF 버튼 제거).
