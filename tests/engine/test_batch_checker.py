...
import pytest
from src.engine.interop.model_schema import FrameModel3D, FrameNode, FrameElement, FrameSection, MemberForce
...
def test_batch_checker_rc_members():
    ...
    model = FrameModel3D()
    # Add sections
    model.sections[1] = FrameSection(sec_id=1, sec_name="B400x600", b=400.0, h=600.0)
    model.sections[2] = FrameSection(sec_id=2, sec_name="C600x600", b=600.0, h=600.0)

    forces_by_elem = {}
    # Create 50 beams and 50 columns
    for i in range(1, 51):
        # Beam
        model.elements[i] = FrameElement(elem_id=i, elem_type="BEAM", mat_id=1, sec_id=1, nodes=[1, 2], story="2F")
        forces_by_elem[i] = [
        ...
        # Column
        col_id = i + 100
        model.elements[col_id] = FrameElement(elem_id=col_id, elem_type="COLUMN", mat_id=1, sec_id=2, nodes=[3, 4], story="1F")
...
def test_batch_checker_story_filter():
    ...
    model.elements[1] = FrameElement(elem_id=1, elem_type="BEAM", mat_id=1, sec_id=1, nodes=[1, 2], story="1F")
    model.elements[2] = FrameElement(elem_id=2, elem_type="BEAM", mat_id=1, sec_id=1, nodes=[2, 3], story="2F")
