...
4. **수퍼 파일(Superfile) 주의 및 점진적 리팩토링 (Pragmatic Modularity)**: 수퍼파일에 주의하되 포팅 흐름을 끊는 인위적 라인 수 강제 제한을 지양하며, 비대해진 파일은 기능 완수 및 오차 검증 후 추후 리팩토링 단계에서 단일 책임 원칙(SRP)에 따라 점진적으로 분할합니다.
...
## 3. 수퍼 파일 주의 및 점진적 리팩토링 가이드라인

1. **주의하되 강제 제한 지양**:
   - 포팅 및 알고리즘 구현 중에는 인터페이스 파편화 방지 및 작업 흐름 유지를 위해 파일 라인 수(예: 500라인)를 강제로 제한하지 않습니다.
   - 단일 책임 원칙(SRP)과 가독성을 항상 염두에 두되, 기능 구현과 0.10% 오차 검증 완료를 최우선으로 진행합니다.
2. **사후 점진적 패키지 승격 (Sub-Package Promotion)**:
   - 포팅 과정에서 비대해진 모듈은 기능 완수 후 리팩토링 단계에서 필요에 따라 단일 파일(`beam.py`) 대신 하위 패키지 디렉토리(`rc/beam/`)로 승격하여 분할:
