...
본 문서는 **AltDP_3rd 모던 웹 부재설계 플랫폼**의 프론트엔드 아키텍처, 4-Pane 반응형 레이아웃, 독립 드래그 리사이저 엔진, 중앙 데이터 스토어(SSOT), 다중 단위계 관리, 파라메트릭 인풋 폼, 2D/3D 벡터 그래픽 엔진, 실시간 KDS 구조계산서 렌더러와 **원본앱 데스크톱 앱의 C++/MFC 역공학 분석(Ground Truth)** 및 상호 1:1 매핑 구현 사항을 총체적으로 집대성한 단일 진실 공급원(SSOT) 종합 명세서입니다.
