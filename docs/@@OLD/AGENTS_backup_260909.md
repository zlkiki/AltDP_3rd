...
* **4대 포팅 참조 우선순위**: **`1순위 추출 소스` > `2순위 매뉴얼/리소스` > `3순위 공인 예제집` > `4순위 국가건설기준`**
  - **3자 삼각 대조**: `[원본 소스/매뉴얼]` ↔ `[신 기준 재계산 예제집]` ↔ `[AltDP_3rd 엔진]` 3자 삼각 대조로 오차 $\le 0.10\%$ 엄수 및 Zero-Dependency 유지.
  - **원문 검증 우선 & 신 기준 재계산 단일 검증자료 채택**: 학회 예제집(3순위) 인용 시 원본 PDF/HWPX 실측 대조 선행. 예제집 풀이 수식이 구 기준일 경우 구 기준 답을 배제하고 **현행 KDS 신 기준 공식으로 재계산된 결과값만을 무조건 단일 검증자료로 채택**하여 3자 삼각 대조 수행 ([`docs/10 제1.4절`](../docs/10_agent_development_protocols.md)).
...
2. **KDS 기준 & 0.1% 오차 무결성 (3자 삼각 대조 & 신 기준 재계산 원칙)**:
   - `[원본 소스/매뉴얼]` $\leftrightarrow$ `[신 기준 재계산 예제집]` $\leftrightarrow$ `[AltDP_3rd 엔진]` 교차 대조 (0.10% 이하 오차).
   - 예제집/기준서 인용 시 원본 PDF/HWPX 실측 렌더링을 통한 **원문 검증 우선(Source-Verification First)** 적용.
   - 예제집이 구 기준일 경우 **현행 KDS 신 기준으로 재계산된 값만을 무조건 단일 검증자료로 확정 채택** (4자 검증 금지). 불일치/오탈자 발견 시 `kcsc2md` 선행 패치 원칙 적용.
